...
Hyena::Hyena(int age, std::string gender, std::string birthSeason,
             std::string color, double weight, std::string origin)
    : Animal(age, std::move(gender), std::move(birthSeason),
            std::move(color), weight, std::move(origin), "Hyena") {
    id = genUniqueID();
}

std::string Hyena::genBirthDay() const {
...
std::string Hyena::genUniqueID() {
    std::stringstream ss;
    ss << "HY" << std::setw(2) << std::setfill('0') << nextId++;
    return ss.str();
}

std::string Hyena::getDescription() const {
    std::stringstream ss;
